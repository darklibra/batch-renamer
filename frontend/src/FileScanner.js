...
const FileScanner = () => {
  ...
  const handleScan = async () => {
    ...
    setError('');
    try {
      const result = await dataProvider.indexFiles(directoryPath);
      setMessage(`스캔 완료: ${result.indexed_files.length}개의 파일이 추가되었습니다.`);
  ...
  return (
    <Card sx={{ margin: '2em' }}>
      <CardHeader title="파일 스캔" />
      <CardContent>
        <Typography variant="body1" gutterBottom>
          스캔할 디렉토리의 절대 경로를 입력하세요.
        </Typography>
        <TextField
          label="디렉토리 경로"
          variant="outlined"
