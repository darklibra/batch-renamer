...
from backend.app.application.use_cases.index_files import IndexFilesUseCase
from backend.app.interfaces.api.dependencies import get_index_files_use_case, get_file_repository, get_apply_patterns_to_specific_file_use_case # get_apply_patterns_to_specific_file_use_case 추가
from backend.app.interfaces.api.v1.dtos.file_dtos import IndexRequest, FileResponse, IndexResponse
from backend.app.domain.file.repository import FileRepository
from backend.app.application.use_cases.extracted_data.apply_patterns_to_specific_file import ApplyPatternsToSpecificFileUseCase # 추가
...
def apply_patterns_to_specific_file(
    file_id: int,
    use_case: ApplyPatternsToSpecificFileUseCase = Depends(get_apply_patterns_to_specific_file_use_case)
):
    extracted_data = use_case.execute(file_id)
    if extracted_data is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="File not found or no patterns applied.")
    
    # 추출 성공 후 업데이트된 파일 정보를 반환
    updated_file = use_case.file_repository.find_by_id(file_id)
    if not updated_file:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Updated file not found.")
